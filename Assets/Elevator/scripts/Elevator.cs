...
public class Elevator : MonoBehaviour
{
    ...
    public void CollectObservations(Agent a)
    {
        ...
        a.AddVectorObs(0);  //남은 승객수

        building.CollectObsFloor(a);

        CollectObsElevator(a);

    }

    public void CollectObsElevator(Agent a)
    {

        int floor, nextfloor;
        GetFloor(out floor, out nextfloor);

        a.AddVectorObs(floor);   ///헌재 층수
        a.AddVectorObs(nextfloor);   ///다음층수
        a.AddVectorObs(moveDir[(int)GetMoveState()]);  ///이동방향
        a.AddVectorObs((int)fsm.GetCurrentState());
        a.AddVectorObs(listPassinger.Count);

        for (int i = 0; i < floorBtnflag.Length; ++i)
        {
            a.AddVectorObs(floorBtnflag[i] ? 1f : 0f);
        }
 ...
 //                  || brain.brainType == BrainType.Player)
    ...
    public bool AddPassinger(ElevatorPassenger p)
    {
        ...
        else  
        {
            ...
            if (floor == 0 || floor == ElevatorAcademy.floors - 1)
            {
                listPassinger.Add(p);
                SetTransitionDelay(Event.DoorCloseStart, Random.Range(0.6f, 1.0f), true);
                SetFloorButton(p.destFloor, true);
    ...
    public void RequstAction(int floor)
    {
        ...
        if(agent.brain.brainType == BrainType.Heuristic
            || agent.brain.brainType == BrainType.Player)
        {
